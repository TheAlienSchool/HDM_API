...
This document is the **single source of truth** for all visible copy across both portals.
...
2. Edit the copy in this document first
3. Apply the change to the HTML at the mapped location
4. Run a syntax check (`node --check`) if editing JS string content

**For agentic handoff:**
Provide this document + the instruction: *"Apply all copy updates in [section name] to the corresponding HTML file using the location keys."*
...
**The exception :: Pedagogical Negation:** Negation is permitted only when addressing a specific, real, pre-existing misconception in the reader's mind. It must be immediately followed by an affirmative statement. It must earn its use.
...
**The Listener's Path :: excluded words** (except in canonical use):
...
*How Markdown here mirrors typographic emphasis in the HTML.*

| Markdown | HTML Element | Usage |
|---|---|---|
| `# ALL CAPS HUGE` | `.display-huge` | Hero/cover title |
| `## Display Large` | `.display-large` | Section anchors |
| `### Display Mid` | `.display-mid` | Statement headlines |
| `> *Italic blockquote*` | `.pull-quote` | Featured quotes |
| `**EYEBROW LABEL**` | `.eyebrow` | Section labels, all-caps small |
| `*Italic text*` | `.felt` / `<em>` | Somatic felt-quotes, emphasis |
| `` `term` `` | `.tip` (tooltip) | HDM or Tradition term w/ tooltip |
| `_Plain English: ..._` | `.oct-card-deep em` | First-timer plain language note |
| `→ FIELD READING` | `.orrery-state` | PING pattern description |
| `---` | `.section-rule` | Visual breathing space |

**Color register in HTML (for reference):**
- `--terra` (terracotta) = HDM terms, eyebrows, emphasis
- `--gold` = Tradition terms, Kai's field, PING™ branding
- `--sand` = Primary display copy
- `--ink-light` = Body text, secondary prose
...
## [SCENE 0] COVER
> 📍 `#cover` → `.cover-top / .cover-main / .cover-meta`

**HUMAN DEVELOPMENT MATHEMATICS · THE ÅĻÏEN SCÖÕL · March 2026**
...
*The Ethnomathematical Lineage of Human Development Mathematics*
...
## [SCENE 1] INVITATION
> 📍 `#invitation` → `.eyebrow + first-timer orient + .inv-statement + .inv-sub`
...
In basket-weaving, in sand-drawing, in wall-engraving, in ceremony :: human development follows `mathematical patterns`. This knowledge lived in ceremony, in the body, in community :: long before any institution arrived to name it.
...
*Hover or tap each numeral to understand its gateway.*

---

## [SCENE 2] LINEAGE
> 📍 `#lineage` → `.eyebrow + .display-mid + .body-text + .pull-quote`

**OCTAVE VI · DECOLONIAL RECOVERY**

### Mathematical thinking was already in every hand
### *before any institution named it.*

`Ethnomathematician` Paulus Gerdes documented mathematical intelligence embedded in cultures across Africa, Asia, and the Americas. His finding: the same structures :: graph theory, symmetry, matrix algebra :: discovered independently, across thousands of miles, with zero contact. The intelligence was native. Each tradition arrived here on its own.

This is the epistemological gate. If mathematics is a pan-human capacity :: and the evidence says it is :: then `HDM` stands as a legitimate formalization of a deep human truth.
...
*`Sona` of Angola · `Litema` of Lesotho · `Mariposas` of Peru · `Kolam` of Tamil Nadu · `Woven mats` of Mozambique*

---

## [SCENE 3] THE STAR
> 📍 `#star` → `.eyebrow + .display-mid + .body-text + .star-count`

**THE STAR THAT CROSSED EVERY OCEAN**

### Nine cultures. Four continents. One mathematical truth. Zero contact.
...
**9** Distinct cultures
**4** Continents
**0** Contact between them

---

## [SCENE 4] THE SEVEN OCTAVES
> 📍 `#octaves` → `.octaves-intro + .octave-grid`

**THE SEVEN OCTAVES · HDM'S CORE ARCHITECTURE**
...
### OCTAVE V · THE EPISTEMOLOGICAL GATE
> 📍 `#octaves` → `.oct-card` [data-numeral=V]

**Pan-Human Intelligence** · *The Constellation*

Mathematical thinking is older than any institution that ever claimed to own it. It was already in every hand, every ritual, every threshold drawing at dawn.

*→ In HDM terms, this is the `curiosity velocity` dimension. The capacity to see connections across what seems separate. When someone's curiosity velocity is high, they recognize the pattern before they can name it. This is the PING™ in action: the flash of recognition before the language arrives.*

_Plain English: Do you believe you're capable of real, original intelligence?_

---

### OCTAVE VI · THE UNFROZEN ARTIFACT
> 📍 `#octaves` → `.oct-card` [data-numeral=VI]
...
_Plain English: Your intelligence has deep roots. You already carry more than you know._

---

### OCTAVE VII · THE BROKEN LINE
> 📍 `#octaves` → `.oct-card` [data-numeral=VII]
...
_Plain English: Something was taken. The recovery is already underway._

---

### OCTAVE IV · THE NARRATIVE ARC
> 📍 `#octaves` → `.oct-card` [data-numeral=IV]
...
_Plain English: Your story and your geometry are the same thing._

---

### OCTAVE III · THE HAND AND THE HEPTAGON
> 📍 `#octaves` → `.oct-card` [data-numeral=III]

**Body as Instrument** · *The Somatics of the Algorithm*

The geometry emerged from the touch. The body was the first instrument of mathematical knowing.

*→ `Surface Tension` is felt in the body before it is named anywhere else. `Trust Velocity` is a physical experience. `Creative Resonance` is a particular quality of aliveness. These are somatic signals before they are variables.*

_Plain English: Your body already knows. HDM gives that knowing a name._

---

### OCTAVE II · THE EMERGENT ALGORITHM
> 📍 `#octaves` → `.oct-card` [data-numeral=II]

**Practice Before Theory** · *The Legitimacy of Lived Experience*
...
_Plain English: This framework is validated by what actually happened to real people._

---

### OCTAVE I · THE UNVEILING
> 📍 `#octaves` → `.oct-card` [data-numeral=I]

**Frozen Mathematics** · *The Claim*

The central argument: HDM is the unfreezing of developmental intelligence that has always been in practice.

*→ The `Stillness Index` is where the unfreeze begins. You stop long enough to see the pattern. `Frozen mathematics` is alive and waiting. HDM is the instrument of recovery, applied to the domain of the self.*

_Plain English: The wisdom of your own development is already inside you. HDM helps you read it._

---

## [SCENE 4.5] DSP SELF-ASSESSMENT
> 📍 `#dsp-snapshot` → `.eyebrow + .display-sm + .body-text + .dsp-slider-row + .dsp-st-box`

**DEVELOPMENT STATE PROFILE**
...
## [SCENE 5] BODY AS INSTRUMENT
> 📍 `#body-scene` → `.eyebrow + .pull-quote + .somatic-item`
...
The felt gap between knowing where you're going and what's in the way. Where purposive clarity meets relational resistance. The most diagnostic metric in HDM.

**`Creative Resonance`**
*"The work has hold of me."*
A particular quality of aliveness in generative work. The felt sense of living fully inside the frequency. Somatic before it is cognitive.
...
## [SCENE 6] THE CLAIM
> 📍 `#claim` → `.eyebrow + .claim-statement + .claim-support + .claim-lineage + .pull-quote`
...
The `Chokwe drawing expert` was practicing developmental mathematics.
The `Sotho wall-engraver` was practicing developmental mathematics.
The `Mwani weaver` was practicing developmental mathematics.
The `Bora basket-weaver` was practicing developmental mathematics.
The `Tamil kolam-drawer` was practicing developmental mathematics.

None of them called it that.
But the mathematics was there, `frozen in the practice`, waiting to be recovered.

> *"It is the latest act of a very long recovery. It is the infrastructure for the inner life of the 21st century."*

---

## [SCENE 6.5] THE GAP
> 📍 `#the-gap` → `.eyebrow + .display-mid + .gap-stats-grid + .gap-bridge + .gap-matrix-title + .gap-matrix-sub`

**THE BROKEN SYSTEM**

### Every tool that came before measures what already happened. HDM reads where someone is going.

> 📍 `.gap-stat` (four animated counters)
...
An invitation to recognize what HDM is, and where it comes from.
...
## [SCENE 7] TRADITIONS CONSTELLATION
> 📍 `#traditions` → `.traditions-intro + .trad-node` (interactive SVG)
...
## [SCENE 8] PING™
> 📍 `#ping-scene` → `.ping-instr-title + .ping-instr-step + .ping-body + .ping-conn-popup`
...
## [SCENE 9] PATTERN WITHOUT JUDGMENT
> 📍 `#orrery-scene` → `.eyebrow + .display-mid + prose + .orrery-default + .orrery-panel`

**PING™ · PATTERN WITHOUT JUDGMENT**
...
*A resonance field · fictional team*

> 📍 `#orrery-default` (default state before node selection)
...
→ *Translation is happening. Vision is becoming form...*
...
→ *Quiet precision. Devon grounds what others generate...*
...
→ *A field turned inward. Steeping. The deepest questions arrive from this place. Kai is preparing something the team will recognize when it arrives.*
Invitation: *This stillness is worth a direct conversation.*

---

## [SCENE 10] CLOSE
> 📍 `#close-scene` → `.eyebrow + .close-headline + .close-sub + .close-koan + .close-links + .close-footer`

**THE ÅĻÏEN SCÖÕL · CREÅTIVE STEEPING**

### The lineage is alive.
### *The recovery continues.*

The Aboriginal elder sang the landscape to navigate it. History remembered in the mind, body, and feet as the stories were danced. The Griot held two thousand years of genealogy in his voice. The Mwani weaver encoded matrix algebra in the movement of her hands.

Every culture in human history has navigated the mathematics of inner development. The knowledge survived. In ceremony. In technique. In the transmission from elder to initiate. In the body, before the body had language for what it knew.
...
**Enter CREÅTIVE STEEPING** ← primary → `http://thealienschool.com`
**Explore the Partnership Brief** ← `href="#"` ⚠️ *PENDING URL*
**Read the White Paper** ← `href="#"` ⚠️ *PENDING URL*

> 📍 `.close-footer`

THE ÅĻÏEN SCÖÕL · Human Development Mathematics · 2026
...
## PREMISE
> 📍 `.eyebrow + .display-mid + .body-prose` (opening section)

**THE PREMISE**

### The Exploratorium explains the mathematics. The Songline teaches it through the body.
...
Together, they form the complete learning architecture of Human Development Mathematics.
...
## GOVERNING QUESTION
> 📍 `.close-question` (Crossover close)
...
Intellectual entry. Evidence-based. Map.

**The Digital Songline** gives the traveler the experience.
*What does this feel like in the body? How do I walk it?*
Somatic entry. Practice-based. Journey.

**The Living Ecosystem** gives the community the continuity.
*Who else is in this field? What does our collective pattern look like?*
Relational entry. Emergence-based. Home.

**The API** makes the whole journey legible over time.
*Where have I been? Where am I going? What is the arc?*
Longitudinal entry. Data-based. Mirror.
...
> ⚠️ These require action before the portals are fully live.

| Item | Location | Status |
|---|---|---|
| Partnership Brief URL | `#close-scene` → `.close-link` (2nd button) | URL needed |
| White Paper URL | `#close-scene` → `.close-link` (3rd button) | URL needed |
| Digital Songline portal | Separate build | Phase II+ |
| Traditions Constellation expansion | `#traditions` → `TRADITIONS` JS data | Phase I pending |
| Songlines node (full) | `TRADITIONS` JS data | Ready to write |
| Griot node (full) | `TRADITIONS` JS data | Ready to write |
| Polynesian Wayfinding node (full) | `TRADITIONS` JS data | Ready to write |
